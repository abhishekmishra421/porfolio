...
<?php echo form_open_multipart('admin/blog_edit/'.$post->id); ?>
<div class="row">
    <div class="col-12">
        <div class="admin-card mb-4">
            <div class="admin-card-header">
                <h5><i class="fas fa-pen me-2" style="color:var(--primary)"></i>Edit Post</h5>
            </div>
            <div class="admin-card-body">
                <div class="mb-3">
                    <label class="form-label">Post Title <span class="text-danger">*</span></label>
                    <input type="text" name="title" class="form-control" value="<?php echo set_value('title', $post->title); ?>" required>
                ...
                <div class="mb-3">
                    ...
                    <textarea name="excerpt" class="form-control" rows="3"><?php echo set_value('excerpt', $post->excerpt); ?></textarea>
                </div>

                <div class="mb-3">
                    <label class="form-label">Main Content <span class="text-danger">*</span></label>
                    <textarea name="content" class="form-control" id="summernote" rows="15" required><?php echo set_value('content', $post->content); ?></textarea>
                    <?php echo form_error('content', '<div class="text-danger" style="font-size:0.8rem;margin-top:4px;">', '</div>'); ?>
                </div>

                <div class="mb-3">
                    <label class="form-label">Author Name</label>
                    <input type="text" name="author" class="form-control" value="<?php echo set_value('author', $post->author); ?>">
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="admin-card mb-4">
            ...
            <div class="admin-card-body">
                <div class="mb-4 form-check form-switch custom-switch" style="padding-left:0;display:flex;align-items:center;justify-content:space-between;">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_featured" value="1" <?php echo ($post->is_featured == 1) ? 'checked' : ''; ?> style="width:40px;height:20px;margin-left:auto;cursor:pointer;">
                </div>

                <div class="mb-4 form-check form-switch custom-switch" style="padding-left:0;display:flex;align-items:center;justify-content:space-between;">
                    <label class="form-check-label" style="color:var(--text);font-weight:600;font-size:0.9rem;">Status (Published/Draft)</label>
                    <input class="form-check-input" type="checkbox" name="is_active" value="1" <?php echo ($post->is_active == 1) ? 'checked' : ''; ?> style="width:40px;height:20px;margin-left:auto;cursor:pointer;">
                </div>
            ...
            <div class="admin-card-body">
                ...
                <?php if($post->featured_image): ?>
                    <div class="mt-3 text-center" style="background:rgba(255,255,255,0.05);padding:10px;border-radius:8px;">
                        <img src="<?php echo base_url('uploads/blog/'.$post->featured_image); ?>" alt="Preview" style="max-width:100%;height:auto;border-radius:6px;">
                    </div>
                <?php endif; ?>
            </div>
        </div>
